classdef  AutoNGCLOSKGuess < userfunutils.UserGuessInterface
    %AutoNGCLOSKGuessObject Computes guess for auto NGC lunar orbit station keeping use case
    ...
    properties
        ...
        initKepState
        % Mu
        gravParamMoon = 4902.8005821478;
    end
    
    methods
        
        function obj = AutoNGCLOSKGuess(state,mass)
            % Constructor
        ...
        function [stateGuess,controlGuess] = GetInitialGuess(obj,timeVector,~,phasePointer)
            ...
            for ii = 1:numTimes-1
                controlGuess(ii,:) = [1 0 -1 0]; % indicates that burn is to reduce energy (in -theta direction)
            end
        ...
        function finalTrueAnomaly = KeplerProp(obj,deltaT)
            ...
            % a delta time
            meanMotion = sqrt(obj.gravParamMoon/obj.initKepState(1)^3);
