classdef ElectricThruster < handle
    ...
    properties  (SetAccess = public)
        ...
        debugMath = false();
        % Integer.  1 if using ThrustDir fields, 2 if using externally set
        % thrust
        ThrustMode = 1;
    end
    ...
    methods
        ...
        function [thrustMag,mDot,powerUsed] = ...
			...
			% Power used should be able to go negative to ensure smoothness, so simply use the
			% allocated power, even if it is not feasible
        ...
        %% Set power allocated
        function power = GetPowerAllocated(obj)
            power = obj.powerAllocated;
        end
        		
        %% Get max power usable
        function power = GetMaxUsablePower(obj)
            power = obj.MaximumUsablePower;
        end
		
        %% Get min power usable
        function power = GetMinUsablePower(obj)
            power = obj.MinimumUsablePower;
        end
        ...
        function thrustUnitVec = GetThrustUnitVec(obj)
			...
			if norm(thrustDir)
				...
				thrustUnitVec = thrustDir;
			end
        end

        %%  Compute thrust vector magnitude in thrust system
        function thrustMag = GetThrustMag(obj)
            thrustMag = norm([obj.ThrustDirection1 obj.ThrustDirection2...
                obj.ThrustDirection3]);
        end
        
        %%  Set Thrust Vector
        function SetThrustVector(obj,thrustVector)
			if obj.ThrustMode == 3 || obj.ThrustMode == 4
				obj.ThrustDirection1 = sin(thrustVector(1))*cos(thrustVector(2));
				obj.ThrustDirection2 = sin(thrustVector(1))*sin(thrustVector(2));
				obj.ThrustDirection3 = cos(thrustVector(1));
				if obj.ThrustMode == 4
                	obj.ThrustMag = thrustVector(3);
                end
			else
	            obj.ThrustDirection1 = thrustVector(1);
	            obj.ThrustDirection2 = thrustVector(2);
	            obj.ThrustDirection3 = thrustVector(3);
			end
        end
        ...
        function [thrustVec,mDot,powerUsed] = ...
            ...
            [thrustMag,mDot,powerUsed] = obj.GetNomThrustandMassFlow();
            
            %  This is the case where the thrust model is set on the
            %  thruster object.
            if obj.ThrustMode == 1
			...
			elseif obj.ThrustMode == 2 || obj.ThrustMode == 3
                %  This is the case were the thrust is passed in and used
                %  "up to unit vector".
                thrustDir = obj.GetThrustUnitVec();
                appliedThrustScaleFactor = obj.GetThrustMag();
                thrustMag = thrustMag*appliedThrustScaleFactor;
                mDot = mDot*appliedThrustScaleFactor;
				
			elseif obj.ThrustMode == 4
                %  This is the case were the thrust is passed in and used
                %  "up to unit vector".
                thrustDir = obj.GetThrustUnitVec();
                appliedThrustScaleFactor = obj.ThrustMag;
                thrustMag = thrustMag*appliedThrustScaleFactor;
                mDot = mDot*appliedThrustScaleFactor;	
            end
            
            thrustVec = obj.DutyCycle*obj.ThrustScaleFactor*thrustMag*thrustDir;
