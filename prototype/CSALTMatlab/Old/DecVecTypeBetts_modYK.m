classdef DecVecTypeBetts_modYK < collutils.DecisionVector
    ...
    properties
        ...
        %% new data types added by YK
        stateInterpType
        stateInterpTypeNames={'Lagrange','cubicHermite'};
        controlInterpType='natrualCubic';
        controlInterpTypeNames={'natrualCubic'};
        % Lagrange or cubic hermite interpolation for state
        % natural cubic spline for control
    ...
    methods
        ...
        %% methods added by YK
        function SetNormalizedStateTimeSeq(obj,normTimeVec)
            % input time vector must be normalized in the interval [-1,1]
            % because time boundaries are alread known in the
            % decisionVector
            
            [numRows, numCols]=size(normTimeVec);
            if numRows ~= 1 && numCols ~= 1
                error('input must be a column or row vector');
            end
            if normTimeVec(1) ~= -1 || normTimeVec(end) ~= 1
                error('the normalized time vector must begin from -1 and end at 1')
            end
            if numel(normTimeVec) ~= obj.numStatePoints
                error('input length must be equal to the number of state points')
            end
            obj.normalizedStateTimeSeq=normTimeVec;                      
        end
        function SetNormalizedControlTimeSeq(obj,normTimeVec)
            % input time vector must be normalized in the interval [-1,1]
            % because time boundaries are alread known in the
            % decisionVector
            ...
            if numel(normTimeVec) ~= obj.numControlPoints
                error('input length must be equal to the number of control points')
            end
            obj.normalizedControlTimeSeq=normTimeVec;                      
        end        
        function SetStateInterpType(obj,interpType)
            if ~ischar(interpType)
                error('input must be a string')
            end
            isValidType=strcmpi(obj.stateInterpTypeNames,interpType); 
            % a boolean array
            
            if sum(isValidType) == 0
                error('invalid interpolation type name')
            end
            obj.stateInterpType=interpType;            
        end
        function newStateArray=InterpolateState(obj,newNormTimeVec,dynArray)
            % input time vector must be normalized in the interval [-1,1]
            % because time boundaries are alread known in the
            % decisionVector
            
            % cubic Hermite interpolation requires dynArray
            if nargin ==2 && strcmp(this.stateInterpType,'cubicHermite')
                error('')
            end
            ...
            if newNormTimeVec(1) < -1 || newNormTimeVec(end) > 1
                error('the range of the normalized time sequence must be [-1,1]')
            end
            
            if strcmp(obj.stateInterpType,'Lagrange')
            % To be done: modify LagrangeInterpolator to address
            % multi-dimensional data
                if numCols ~=1
                    newNormTimeVec=newNormTimeVec.';
                end
                [~,numCols]=size(obj.normalizedStateTimeSeq);
                if numCols ~=1
                    normalizedStateTimeSeq=obj.normalizedStateTimeSeq.';
                else
                    normalizedStateTimeSeq=obj.normalizedStateTimeSeq;
                end
                
                theInterp = LagrangeInterpolator;
                theInterp.SetIndependentVariables(normalizedStateTimeSeq);
                stateArray=GetStateArray(obj);
                newStateArray=zeros(numel(newNormTimeVec),obj.numStateVars);
                for i=1:obj.numStateVars
                    for j=1:numel(newNormTimeVec);
                        newStateArray(j,i) = theInterp.Interpolate(newNormTimeVec(j),stateArray(:,i));            
                    end
                end                
            elseif strcmp(obj.stateInterpType,'cubicHermite')            
                % to be done: how to obtain dynArray?
                newStateArray= CubicHermiteSpline(obj.normalizedStateTimeSeq,GetStateArray(obj),dynArray,newNormTimeVec);                            
            end            
        end
        function newControlArray=InterpolateControl(obj,newNormTimeVec)
            % input time vector must be normalized in the interval [-1,1]
            % because time boundaries are alread known in the
            % decisionVector
            ...
            % to be done: interpolation based on natrual cubic spline
            % 'spline.m'
            
            % in 'spline', the data must be column-sequential.
            newControlArray=spline(obj.normalizedControlTimeSeq,GetControlArray(obj).',newNormTimeVec).';            
        end        
    end
