...
%  Test setting scaling based on Jacobian values
scaler.SetConstraintScaling_Jacobian(A)
scaledJac = scaler.ScaleJacobian(sparseJac);
jacTest2 = scaler.UnScaleJacobian(scaledJac) - sparseJac; 
disp('The scaled Jacobian is')
disp(scaledJac)
...
if max(jacTest2) >= 1e-12
    disp('Error in scaling or unscaling of Jacobian')
else
    disp('Jacobian scaling passed example 2 test')
end