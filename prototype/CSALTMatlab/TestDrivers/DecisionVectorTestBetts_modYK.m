...
import collutils.DecVecTypeBetts_modYK
...
myVector = DecVecTypeBetts_modYK();
...
% test 1 set state interp type
myVector.SetStateInterpType('cubicHermite')
myVector.SetStateInterpType('Lagrange')

% test 2 set new normalized state time squence 
stateTimeSeq=linspace(-1,1,numStatePoints);
myVector.SetNormalizedStateTimeSeq(stateTimeSeq);

% test 3 set new normalized control time squence 
controlTimeSeq=linspace(-1,1,numControlPoints);
myVector.SetNormalizedControlTimeSeq(controlTimeSeq);

% test 4 interpolate control
newControlTimeSeq=(controlTimeSeq(1:end-1)+controlTimeSeq(2:end))/2;
controlArray=myVector.GetControlArray();
newControlArray=myVector.InterpolateControl(newControlTimeSeq);
for i=1:myVector.numControlVars
    figure;plot(controlTimeSeq,controlArray(:,i), newControlTimeSeq,newControlArray(:,i));
end

% test 5 interpolate state with cubic hermite
% it does not work yet!
% need to get dynArray!

stateArray=myVector.GetStateArray();
% myVector.SetStateInterpType('cubicHermite')
newStateTimeSeq=(stateTimeSeq(1:end-1)+stateTimeSeq(2:end))/2;
% newStateArray1=myVector.InterpolateState(newStateTimeSeq);
% for i=1:myVector.numStateVars
%     figure;plot(stateTimeSeq,stateArray(:,i), newStateTimeSeq,newStateArray1(:,i));
% end
% test 6 interpolate state with Lagrange.

myVector.SetStateInterpType('Lagrange')
newStateArray2=myVector.InterpolateState(newStateTimeSeq);
for i=1:myVector.numStateVars
    figure;plot(stateTimeSeq,stateArray(:,i), newStateTimeSeq,newStateArray2(:,i));
end
% 
