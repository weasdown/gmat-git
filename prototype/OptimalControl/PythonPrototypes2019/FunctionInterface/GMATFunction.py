...
from TransformUtil import TransformUtil
...
class GMATFunction(AlgebraicFunction.AlgebraicFunction):
    ...
    def __init__(self):
        ...
        self.transformUtil = TransformUtil()
    ...
    def GetUnscaledStateForFun(self, pointIdx, returnElements = ['All']):
        ...
        if orbitConversion:
            ...
            # new version, exported to TransformUtil
            unscaledState[idxOrbStart:idxOrbEnd] = self.transformUtil.TransformOrbitalState(
                                                                                            unscaledState[idxOrbStart:idxOrbEnd],
                                                                                            self.GetTime(pointIdx),
                                                                                            phaseConfig.Origin,
                                                                                            phaseConfig.Axes,
                                                                                            phaseConfig.OrbitStateRep,
                                                                                            funConfig.Origin,
                                                                                            funConfig.Axes,
                                                                                            funConfig.OrbitStateRep,
                                                                                            'Spline',
                                                                                            self.ephemSmoother)

            ## old version, done all in-house
    ...
    def GetUnscaledJacobianOfFunVarsWrtDecVarsAndTime(self, pointIdx, returnElements = ['All']):
        ...
        if orbitConversion:
            ...
            # new version, exported to TransformUtil
            jacOrbitStateFuncVarsWrtPhaseVars, jacOrbitStateFuncVarsWrtTime = self.transformUtil.CalculateOrbitalJacobian(
                                                                                            unscaledState[idxOrbStart:idxOrbEnd],
                                                                                            self.GetTime(pointIdx),
                                                                                            phaseConfig.Origin,
                                                                                            phaseConfig.Axes,
