...
class srp_validation(object):
    ...
    def get_d_a_srp_d_x(self, t, r):
        ...
        rvs = r - rs 
        rvsmag = np.linalg.norm(rvs)
        coef1 = nu * self.P_s * rsmag**2 * self.C_r * self.Area / self.mass / rvsmag**3
        coef2 = self.P_s * rsmag**2 * self.C_r * self.Area / self.mass / rvsmag**3 * rvs
        eye = np.identity(3)
    ...
    def get_A(self, t, r):
        ...
        c1 = self.get_c1(t, r)
        c2 = self.get_c2(t, r)
        #A = rad_sun_app**2 * np.arccos(c1 / rad_sun_app) + rad_body_app**2 * np.arccos((dprime - c1) / rad_body_app) - dprime * c2
...
if __name__ == "__main__":
    ...
    r = np.array([[50.e6 + 6378.0 + 10000000.0], [0.0], [0.0]])
    ...
    print('Position = ', np.transpose(r[:,0]))
    print('Shadow = ', p)
    print('d [Shadow] / d [t] analytical = ', d_p_d_t[0,0])    
    print('d [Shadow] / d [t] AD = ', d_p_d_t_ad)
    print('d [Shadow] / d [r] analytical = ', d_p_d_r)
    ...
    print('')
    print('')
    print('acc = ', np.transpose(a[:,0]))
    print('')
    print('d [Acc] / d [t] analytical = ', d_a_d_t)
    print('')
    print('d [Acc] / d [t] AD = ', d_a_d_t_ad)
    print('')
    print('d [Acc] / d [r] analytical = ', d_a_d_r)
    print('')
