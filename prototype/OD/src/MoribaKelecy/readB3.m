...
fprintf('\n')
Infile = 'Tdrs5_sim3.obs'; %input('  Enter input path and B3 file name => ','s');

%...define output file path and name
%%fprintf('\n')
%%Outfile = input('  Enter output path and B3 file name => ','s');
%%fprintf('\n')


%...assign input file id
fid_in = fopen(Infile);
%...assign output file id
%%fid_out = fopen(Outfile,'w');

Nobs = 0;
fprintf('Reading in B3 data...\n');
while( 1 )
