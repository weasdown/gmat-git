...
state = OrbitState();
state.SetKeplerianState(6900,0.000,90*pi/180,0,0,pi/2);

% Create a conical sensor
coneHalfAngle = 30*pi/180;
conicalSensor = ConicalSensor(coneHalfAngle);

%Create a custom sensor with FOV of coneAngle
numPoints = 20;
coneAngleVec = zeros(numPoints,1);
clockAngleVec = zeros(numPoints,1);
for pointIdx = 1:numPoints
    coneAngleVec(pointIdx) = coneHalfAngle;
    clockAngleVec(pointIdx) = 2*pi/(numPoints)*pointIdx;
end
customSensor = CustomSensor(coneAngleVec,clockAngleVec);

% Create the tropics sensor
coneAngleVec = [
         0.994992207304526
         0.994992207304526
         0.994992207304526
...
clockAngleVec =[0.0218144263689604
          3.11977822722083
          3.16340707995875
       -0.0218144263689604
        0.0218144263689604];
...
count = 0;
while date.GetJulianDate() < startDate + 5
    ...
    if showPlots
        ...
        if count >= 2
            ...
            plot3(orbitState(count-1:count,1),orbitState(count-1:count,2),...
                orbitState(count-1:count,3),'k-','MarkerSize',3)
            plot3(orbitState(count,1),orbitState(count,2),orbitState(count,3),'r*','MarkerSize',3)
        end
    end
...
coverageEvents = covChecker.ProcessCoverageEvents();
% covChecker.ComputeCoverageStatistics()
% covChecker.WriteCoverageReport()
% toc
