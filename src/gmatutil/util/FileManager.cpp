...
FileManager::FILE_TYPE_STRING[FileTypeCount] =
{
   ...
   "SCHATTEN_FILE",
   "MARINI_TROPO_FILE",
   "EARTH_EXPONENTIAL_FILE",
   "MARS_EXPONENTIAL_FILE",
   "HELP_FILE",};
...
#ifdef DEBUG_FIND_PATH
   MessageInterface::ShowMessage("   FileManager::FindPath:  fullName = %s\n", fullname.c_str());
#endif
...
#ifdef DEBUG_FIND_PATH
   MessageInterface::ShowMessage("   FileManager::FindPath: after parse:\n pathOnly = %s\n fileOnly = %s\n gmatPath = %s\n", pathOnly.c_str(), fileOnly.c_str(), gmatPath.c_str());
#endif
   ...
   if (GmatFileUtil::IsPathAbsolute(fullname))
   {
      ...
      else
      {
         ...
         if (forInput)
         {
            ...
            // return empty string instead of generate error message
            //else
            //{
            //   throw UtilityException("Error: The input file " + fullname + " does not exist.\n");
            //}
         }
         else // for output
         {
