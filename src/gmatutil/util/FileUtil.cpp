...
#include "FileManager.hpp"         // for access to some path data
...
//------------------------------------------------------------------------------
/**
 * Looks for a file with a specified file name
 *
 * THe search is performed by looking for the file first relative to the active
 * script location, then relative to the GMAT application folder, and finally
 * in the GMAT output folder.  The path and file name of the first located
 * occurrence of the file is returned.  If the file is not found, an empty
 * string is returned.
 *
 * @param filename The file name as it was provided to the system.
 * @param withBin Include the bin folder in the search
 * @param withOutput Include the output folder in the search
 *
 * @return The full path and filename for the file, or "" is the file was not
 *         found.
 */
//------------------------------------------------------------------------------
std::string GmatFileUtil::FindFile(const std::string &filename, bool withBin, bool withOutput)
{
   std::string retval = "";
   GmatGlobal *globals = GmatGlobal::Instance();
   FileManager *fileman = FileManager::Instance();
   std::string thePath;

   #ifdef DEBUG_FILE_SEARCH
      MessageInterface::ShowMessage("FindFile: Looking for %s\n",
            filename.c_str());
   #endif

   // Absolute paths - potential issue if the path free file is in the GMAT bin folder
   ...
   if (retval == "")
   {
      thePath = fileman->GetGmatWorkingDirectory();
      #ifdef DEBUG_FILE_SEARCH
         MessageInterface::ShowMessage("   Script folder: %s\n",
               thePath.c_str());
      #endif

      if (DoesFileExist(thePath + "/" + filename))
         retval = thePath + "/" + filename;
   }

   // Check bin folder
   if ((retval == "") && withBin)
   {
      thePath = fileman->GetCurrentWorkingDirectory();
      #ifdef DEBUG_FILE_SEARCH
         MessageInterface::ShowMessage("   GMAT folder: %s\n",
               thePath.c_str());
      #endif
   ...
   if ((retval == "") && withOutput)
   {
      // Check output folder
      thePath = fileman->GetFullPathname(FileManager::OUTPUT_PATH);
      #ifdef DEBUG_FILE_SEARCH
         MessageInterface::ShowMessage("   Output folder: %s\n",
               thePath.c_str());
      #endif
                                                   ...
                                                   bool addPath)
{
   ...
   //MessageInterface::ShowMessage("==> pathName=<%s>, fileExt=<%s>\n", pathName.c_str(), fileExt.c_str());
   ...
   {
      throw UtilityException("The directory " + pathName + " does not exist.\n");
   }
   HANDLE hFind;
   ...
   if(hFind == INVALID_HANDLE_VALUE)
   {
      ...
      {
         MessageInterface::ShowMessage
            ("There are no files with the file extension \"" +fileExt+"\" in the directory matching"+ pathName +  "\n" );
      }
      else
      {
         MessageInterface::ShowMessage
...
if (!hasError)
{
   ...
   {
      outFile = entry->d_name;
      if (ParseFileExtension(outFile) == fileExt)
      {
         if (addPath)

            outFile = pathName + entry->d_name;
         fileList.push_back(outFile);
 ...
 * @return  String array containing text lines from the file
 */
//------------------------------------------------------------------------------
