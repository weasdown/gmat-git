...
//#define DEBUG_DIVIDE
//#define DEBUG_PSUEDO_INVERSE_OF_SYMMETRIC_MATRIX
...
//-----------------------------------------------------------------------------
// Rmatrix PsuedoInverseOfSymmetricMatrix(Real zeroValue) const
//-----------------------------------------------------------------------------
/**
* This function is used to calculate psuedo inverse of a symmetric matrix
*/
//-----------------------------------------------------------------------------
Rmatrix Rmatrix::PsuedoInverseOfSymmetricMatrix(Real zeroValue) const
{
   if ((GetNumRows() == 0) || (GetNumColumns() == 0))
      throw UtilityException("Rmatrix::PsuedoInverseOfSymmetricMatrix() can not use to calculate psuedo inverse for an empty matrix.\n");

   if (GetNumRows() != GetNumColumns())
      throw UtilityException("Rmatrix::PsuedoInverseOfSymmetricMatrix() can not use to calculate psuedo inverse for an asymmetric matrix.\n");

   // Step 1: seperate the set of independent columns and the set of dependent columns in this matrix 
   Integer dimA = GetNumColumns();
   IntegerArray R, D;
   std::vector<Rvector> E;
   Real maxA = 1.0;

   for (Integer i = 0; i < dimA; ++i)
   {
      // Check column i in this matrix whether it is indepenent or not
      Rvector u = GetColumn(i);
      for (Integer j = 0; j < R.size(); ++j)
      {
         u = u - (E[j] * u)*E[j];
      }

      if (u.GetMagnitude()/maxA < zeroValue)
         D.push_back(i);
      else
      {
         R.push_back(i);
         Rvector ei = u.GetUnitRvector();
         maxA = GmatMathUtil::Max(maxA, u.GetMagnitude());
         E.push_back(ei);
      }
   }
...
#ifdef DEBUG_PSUEDO_INVERSE_OF_SYMMETRIC_MATRIX
   MessageInterface::ShowMessage(" n = %d  r = %d  R = [", R.size(), r);
   for (Integer i = 0; i < R.size(); ++i)
      MessageInterface::ShowMessage("%d  ", R[i]);
   MessageInterface::ShowMessage("]\n");
#endif


   // At this point, R is permutation index of rows and column
   //                r is range of this matrix
   //                R[i] for i = 1 to r are the indies of independent columns in this matrix
   //                R[i] for i = r+1 to n are the indies of dependent columns in this matrix


   // Step 2: Specify permutation matrix P
   Rmatrix rowP(dimA, dimA);
   for (Integer row = 0; row < dimA; ++row)
   {
      // Fill out row vector of matrix P
      for (Integer col = 0; col < dimA; ++col)
      {
         if (col == R[row])
            rowP(row, col) = 1.0;
         else
...
#ifdef DEBUG_PSUEDO_INVERSE_OF_SYMMETRIC_MATRIX
   MessageInterface::ShowMessage("rowP = [\n");
   for (Integer i = 0; i < dimA; ++i)
   {
      for (Integer j = 0; j < dimA; ++j)
         MessageInterface::ShowMessage("%.1f   ", rowP(i,j));
      MessageInterface::ShowMessage("\n");
   }
   MessageInterface::ShowMessage("]\n");
#endif

   Rmatrix colP = rowP.Transpose();

   // Calculate nxn matrix A
   Rmatrix A = rowP * (*this);
#ifdef DEBUG_PSUEDO_INVERSE_OF_SYMMETRIC_MATRIX
   MessageInterface::ShowMessage("A row = [\n");
   for (Integer i = 0; i < dimA; ++i)
   {
      for (Integer j = 0; j < dimA; ++j)
         MessageInterface::ShowMessage("%.1f   ", A(i, j));
      MessageInterface::ShowMessage("\n");
   }
   MessageInterface::ShowMessage("]\n");
#endif
   ...
   A = A*colP;
#ifdef DEBUG_PSUEDO_INVERSE_OF_SYMMETRIC_MATRIX
   MessageInterface::ShowMessage("A col = [\n");
   for (Integer i = 0; i < dimA; ++i)
   {
      for (Integer j = 0; j < dimA; ++j)
         MessageInterface::ShowMessage("%.1f   ", A(i, j));
...
#ifdef DEBUG_PSUEDO_INVERSE_OF_SYMMETRIC_MATRIX
   MessageInterface::ShowMessage("A1 = [\n");
   for (Integer i = 0; i < r; ++i)
   {
      for (Integer j = 0; j < r; ++j)
         MessageInterface::ShowMessage("%.1f   ", A1(i, j));
      MessageInterface::ShowMessage("\n");
   }
   MessageInterface::ShowMessage("]\n");
#endif
   ...
   // Step 4: Calculate inverse of A1
   Rmatrix invA1 = A1.Inverse();

   // Step 5: Calculate nx(n-r) matrix Alpha
   Rmatrix A2(r, dimA - r);
   for (Integer i = 0; i < r; ++i)
      for (Integer j = r; j < dimA; ++j)
         A2(i, j - r) = A(i, j);
   Rmatrix Alpha = invA1 * A2;

#ifdef DEBUG_PSUEDO_INVERSE_OF_SYMMETRIC_MATRIX
   MessageInterface::ShowMessage("A2 = [\n");
   for (Integer i = 0; i < r; ++i)
   {
      for (Integer j = 0; j < dimA-r; ++j)
         MessageInterface::ShowMessage("%.1f   ", A2(i, j));
      MessageInterface::ShowMessage("\n");
   }
   MessageInterface::ShowMessage("]\n");
#endif

   // Step 6: Specify nxn matrices W and M+
   Rmatrix W(dimA, dimA);
   Rmatrix Mplus(dimA, dimA);
   for (Integer i = 0; i < dimA; ++i)
   {
      for (Integer j = 0; j < dimA; ++j)
      {
         // Calculate nxn matrix W
         if ((i < r) && (j >= r))
            W(i, j) = Alpha(i, j - r);
         else
            W(i, j) = ((i == j) ? 1.0 : 0.0);

         // Calculate nxn matrix M+
         if ((i < r) && (j < r))
            Mplus(i, j) = invA1(i, j);
         else
            Mplus(i, j) = 0.0;
   ...
   // Step 7: Specify matrices U, V, U+, V+, and psuedo inverse of this matrix 
   Rmatrix V = W * colP.Transpose();
   Rmatrix U = rowP.Transpose() * W.Transpose();
   Rmatrix Uplus = (U.Transpose()*U).Inverse()*U.Transpose();
   Rmatrix Vplus = (V.Transpose()*V).Inverse()*V.Transpose();
   Rmatrix Aplus = Vplus * Mplus * Uplus;
...
//------------------------------------------------------------------------------
Rmatrix Rmatrix::Pseudoinverse(Real zeroValue) const
{
   if (isSizedD == false)
   {
      throw TableTemplateExceptions::UnsizedTable();
   ...
   if (rowsD < colsD) 
   {
      ...
      if (!GmatMathUtil::IsZero(lu.Determinant(m1),accuracyRequired))
         InverseM = Transpose()*m1.Inverse(zeroValue);
      else
         InverseM = Transpose()*m1.Inverse(zeroValue);
         //throw Rmatrix::IsSingular();
   } 
   else if (rowsD > colsD) 
   {
      ...
      if (!GmatMathUtil::IsZero(lu.Determinant(m2), accuracyRequired))
         InverseM = m2.Inverse(zeroValue)*Transpose();
      else 
         InverseM = m2.Inverse(zeroValue)*Transpose();
         //throw Rmatrix::IsSingular();
   }
   else
      InverseM = Inverse(zeroValue);
   return InverseM;
}
