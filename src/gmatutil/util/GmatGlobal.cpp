...
//------------------------------------------------------------------------------
// GmatEpoch GmatGlobal::SetCurrentEpoch()
//------------------------------------------------------------------------------
/**
 * Sets an earliest base epoch for the run
 *
 ...
 */
//------------------------------------------------------------------------------
void GmatGlobal::SetCurrentEpoch(const GmatEpoch theEpoch)
{
...
//------------------------------------------------------------------------------
// GmatEpoch GmatGlobal::GetCurrentEpoch()
//------------------------------------------------------------------------------
/**
 * Retrieves an earliest base epoch for the run
 *
...
void GmatGlobal::SetWritePersonalizationFile(bool flag)
{
   writePersonalizationFile = flag;
}

bool GmatGlobal::GetWritePersonalizationFile()
{
   return writePersonalizationFile;
}
...
// Handlers for solver status and state during a run

//------------------------------------------------------------------------------
// void SetSolverStatus(const std::string &solverName, Integer solverStatus)
//------------------------------------------------------------------------------
/**
 * Sets the most recent status for a given solver
 *
 * @note The return is based on name.  In GMAT, solvers are cloned for use.
 *       That meands that there may be several uses of a configured Solver in
 *       the script.  The most status from the most recently used clone is
 *       returned.
 *
 * @param solverName The name of the solver
 * @param solverStatus The status
 */
//------------------------------------------------------------------------------
void GmatGlobal::SetSolverStatus(const std::string &solverName, Integer theStatus)
{
   solverStatus[solverName] = theStatus;
}


//------------------------------------------------------------------------------
// Integer GmatGlobal::GetSolverStatus(const std::string &forSolver)
//------------------------------------------------------------------------------
/**
 * Retrieves teh (Integer) solver status
 *
 * @param forSolver The name of the solver
 *
 * @return The status
 */
//------------------------------------------------------------------------------
Integer GmatGlobal::GetSolverStatus(const std::string &forSolver)
{
   Integer retval = -10;       // Either solver isn't in config, or hasn't been used

   if (solverStatus.find(forSolver) != solverStatus.end())
      retval = solverStatus[forSolver];
...
//------------------------------------------------------------------------------
// std::string GmatGlobal::GetSolverStatusString(const std::string &forSolver)
//------------------------------------------------------------------------------
/**
...
std::string GmatGlobal::GetSolverStatusString(const std::string &forSolver)
{
   ...
   Integer statusVal = GetSolverStatus(forSolver);
...
//------------------------------------------------------------------------------
// void GmatGlobal::ClearSolverStatus()
//------------------------------------------------------------------------------
/**
 * Clears the solver status.
 *
 * This method is called by the Sandbox during initialization.
 */
//------------------------------------------------------------------------------
void GmatGlobal::ClearSolverStatus()
{
...
//------------------------------------------------------------------------------
/**
* Sets the skip splash screen mode on startup
*
* @param tf true to turn on skip splash screen, false to turn it off
*/
//------------------------------------------------------------------------------
void GmatGlobal::SetSkipSplashMode(bool tfSplash)
{
   skipSplash = tfSplash;
}
...
//------------------------------------------------------------------------------
/**
* Returns the current skip splash screen mode
*
* @return true if the mode is set to not display the splash screen on startup
*/
//------------------------------------------------------------------------------
bool GmatGlobal::SkipSplashMode()
{
   return skipSplash;
}
...
{
   // GMAT build date and time
   gmatBuildDate = __DATE__;
   gmatBuildTime = __TIME__;
   ...
   baseEpoch = 21545.0;
   currentEpoch = -1.0;       // Only set current epoch when it is requested
