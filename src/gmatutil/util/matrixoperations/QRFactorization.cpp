...
*/
//------------------------------------------------------------------------------
QRFactorization::QRFactorization(const QRFactorization &copy) :
   MatrixFactorization(copy),
   usePivot(copy.usePivot),
   c(copy.c),
   s(copy.s)
{
   permuMatrix.SetSize(copy.permuMatrix.GetNumRows(),
      copy.permuMatrix.GetNumColumns());
   permuMatrix = copy.permuMatrix;
}
                                            ...
                                            &copy)
{
   if (this != &copy)
   {
      ...
      MatrixFactorization::operator=(copy);
...
void QRFactorization::Factor(const Rmatrix &A, Rmatrix &R, Rmatrix &Q)
{
   ...
   // Force diagonal of R to be positive so it is unique and modify Q
   // accordingly
   Rmatrix signChangeMat = Rmatrix::Identity(m);
   for (Integer ii = 0; ii < n; ++ii)
   {
      if (R(ii, ii) < 0.0)
         signChangeMat(ii, ii) = -1.0;
   }

   R = signChangeMat * R;
   Q = Q * signChangeMat;

   // Set lower triangular elements of R to zero since roundoff from 
   // Givens can sometimes generate values that are not quite zero
...
*/
//------------------------------------------------------------------------------
void QRFactorization::Givens(const Real &el1, const Real &el2)
{
   Real tau = 0.0;
   if (el2 == 0)
   {
