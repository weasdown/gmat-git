   ...
   Real tolerance)
{
   ...
   Real epsilon = tolerance;                                          // Fix bug GMT-7274
   for (int index = 0; index < iSize; index++)
   {
      ...
      {
         //if (isRowOfZeros && infMatrix(index, j) != 0)              // Fix bug GMT-7274
         if (isRowOfZeros && (fabs(infMatrix(index, j)) > epsilon))   // Fix bug GMT-7274
            isRowOfZeros = false;
         //if (isColOfZeros && infMatrix(j, index) != 0)              // Fix bug GMT-7274
         if (isColOfZeros && (fabs(infMatrix(j, index)) > epsilon))   // Fix bug GMT-7274
            isColOfZeros = false;
