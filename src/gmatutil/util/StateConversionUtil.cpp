...
Real StateConversionUtil::CalculateEccentricAnomalyParabola(Real e, Real M)
{
   ...
   throw UtilityException("Calculating the Eccentric Anomaly of a Parabola has not been implemented\n");
                                               ...
                                               Real v[3], AnomalyType anomalyType)
{
   ...
   if (onePlusECos == 0.0)
   {
      // Error due to infinitive value of variable rad
      return 3;
   }
   else if (onePlusECos < ORBIT_TOL)
   {
      if (ecc <= 1.0)
      {
         std::string warn = "Warning: The calculated distance from the central body is "
               "large in the conversion from Keplerian elements to Cartesian "
               "state, and the state may be near a singularity, causing "
               "numerical errors in state conversion.\n";

         MessageInterface::ShowMessage(warn);
      }
      else  // hyperbolic
      {
         Real asymptoteAngle = acos(-1.0/ecc);
         if ((anom >= asymptoteAngle) && (anom <= GmatMathConstants::TWO_PI - asymptoteAngle))
         {
            std::stringstream msg;
            Real ta = asymptoteAngle * DEG_PER_RAD;
            msg << "Error: The true anomaly " << elem[5]
                << " does not fall on the hyperbola defined by eccentricity "
                << ecc << "; valid true anomalies cannot fall between "
                << ta << " and " << (360.0 - ta) << " degrees.";
   ...
   if (rad == std::numeric_limits<Real>::infinity())
   {
      // Error due to infinitive value of variable rad
      return 3;
   }
