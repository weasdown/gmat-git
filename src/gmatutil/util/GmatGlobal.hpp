...
public:
   ...
   std::string GetGMATBuildDate() { return gmatBuildDate; };
   std::string GetGMATBuildTime() { return gmatBuildTime; };
   ...
   void SetCurrentEpoch(const GmatEpoch theEpoch);
   GmatEpoch GetCurrentEpoch();

   Integer GetRunModeStartUp(){return (isTesting? GmatGlobal::TESTING: runMode);};     // It is a temporary fix in order to run regression test with runmode = TESTING.
   ...
   // As noted above, testing mode can be overridden.  This method accesses
   // RUN_MODE = TESTING set in the startup file
   //
   // Suggestion: Replace GetRunModeStartUp() with this call?  At least in some places.
   bool InTestingMode() { return isTesting; };
   // Not used in GMAT, but provides a mechanism for other UIs to change test
   // mode after the startup file is parsed
   void SetTestingMode(bool mode) { isTesting = mode; };
   ...
   void SetSolverStatus(const std::string &solverNmae, Integer solverStatus);
   Integer GetSolverStatus(const std::string &forSolver);
   std::string GetSolverStatusString(const std::string &forSolver);
   void ClearSolverStatus();
...
private:
   ...
   /// Last state of targeters, optimizers, and estimators during a run
   std::map<std::string, Integer> solverStatus;
   ...
   /// GMAT build date
   std::string gmatBuildDate;
   /// GMAT build time
   std::string gmatBuildTime;
