                                         ...
                                         bool checkOuterBracket)
{
   ...
   if (str1[firstOpen] != bracketPair[0] || str1[lastClose] != bracketPair[1])
   {
      ...
      {
         // Reworked with updates from OrbitalQuark
         std::stringstream errorMsg;

         errorMsg << "\"" << str1 << "\" is not enclosed with \""
                  << bracketPair << "\"";
         
         #if DEBUG_SEP_BRACKETS
            MessageInterface::ShowMessage("*** ERROR *** "
                  "GmatStringUtil::SeparateBrackets() %s\n",
                  errorMsg.str().c_str());
         #endif

         throw UtilityException(errorMsg.str());
      }
   }
...
StringArray GmatStringUtil::ParseFunctionCall(const std::string &str)
{
   ...
   else // Args are not a simple [ ] pair, but at least one was found
   {
      ...
      do
      {
         ...
         else  // Handle dangling end of the arg list (commaloc == bstart == bend == npos)
         {
            piece = argstr;
            argstr = "";
         }
...
bool GmatStringUtil::IsBlank(const std::string &text, bool ignoreEol)
{
   ...
   str = Trim(str, GmatStringUtil::BOTH);
   if (str == "")
      return true;
   else
      return false;
...
bool GmatStringUtil::IsValidIdentity(const std::string &str)
{
   ...
   {
      //if (!isalnum(str[i]) && (str[i] != '_') && (str[i] != '.'))    // allow full name such as "P1.AreaCoefficient"
