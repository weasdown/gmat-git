...
public:
   ...
   virtual std::vector<StringArray>& GetDSNCorrectionDatabase();
   ...
   virtual bool           SetDSNCorrectionDatabase(const std::vector<StringArray> &DSNCorrectionDB);
...
protected:
   ...
   /// Global run epoch, used for time management if script sets the CB epoch
   GmatEpoch                runEpoch;
   ...
   // TRK-2-23 Parameters
   StringArray               DSNMediaFileDirectories;
...
private:
   ...
   bool isFirstTimeFlattening;

   bool isGmatTimeWarning;

   //Has the user set these variables?
   bool userSetMu;
   bool userSetFlattening;
   bool userSetEquitorialRadius;
   bool userSetOrientation;
   
   //Have we printed a warning saying the user should set these variables?
   bool defaultOrientationWarningMesssageSent;
