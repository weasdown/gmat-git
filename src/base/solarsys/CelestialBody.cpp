...
CelestialBody::PARAMETER_TYPE[CelestialBodyParamCount - SpacePointParamCount] =
{
   ...
   Gmat::STRINGARRAY_TYPE, //"DSNMediaFileDirectories"
};
...
CelestialBody::CelestialBody(std::string itsBodyType, std::string name) :
   ...
   runEpoch           (0.0),        // Only changed if epoch is set on the CB
   theCentralBodyName (""),
   theCentralBody     (NULL),
   ...
   orientation        (Rvector6(0.0, 0.0, 90.0, 0.0, 0.0, 0.0)),
   naifIdSet          (false),
   naifName           (name),
   ...
   isFirstTimeFlattening                  (true),
   userSetMu                              (false),
   userSetFlattening                      (false),
   userSetEquitorialRadius                (false),
   userSetOrientation                     (false),
   defaultOrientationWarningMesssageSent  (false)
{
...
CelestialBody::CelestialBody(const CelestialBody &cBody) :
   ...
   DSNMediaFileDirectories  (cBody.DSNMediaFileDirectories),
   DSNCorrectionDataBase      (cBody.DSNCorrectionDataBase),
   isFirstTimeMu                       (cBody.isFirstTimeMu),
   isFirstTimeRadius                   (cBody.isFirstTimeRadius),
   isFirstTimeFlattening               (cBody.isFirstTimeFlattening),
   userSetMu                           (cBody.userSetMu),
   userSetFlattening                   (cBody.userSetFlattening),
   userSetEquitorialRadius             (cBody.userSetEquitorialRadius),
   userSetOrientation                  (cBody.userSetOrientation),
...
bool CelestialBody::Initialize()
{
   ...
   if (userDefined)
   {
      if (!defaultOrientationWarningMesssageSent)
      {
         if (!userSetOrientation)
         {
            std::string warnMsg = "WARNING: Orientation parameters have not been set for the user defined body " + instanceName + ". Using defaults.\n";
            MessageInterface::ShowMessage(warnMsg);
            defaultOrientationWarningMesssageSent = true;
         }
      }
   }
...
{
   Real theEpoch = (runEpoch == GmatGlobal::Instance()->GetCurrentEpoch() ?
         lastEphemTime.Get() : GmatGlobal::Instance()->GetCurrentEpoch());
   ...
   return theEpoch; // lastEphemTime.Get();
}
...
Real CelestialBody::SetEpoch(const Real ep)
{
   ...
   runEpoch = GmatGlobal::Instance()->GetCurrentEpoch();
...
//------------------------------------------------------------------------------
// bool CelestialBody::SetDSNCorrectionDatabase(std::vector<StringArray> DSNCorrectionDB)
//------------------------------------------------------------------------------
/**
* Accessor used to set the DSNCorrectionDatabase parameter of the  object.
*
* @param DSNCorrectionDB the information from the .csp files, parsed
*
...
bool CelestialBody::SetDSNCorrectionDatabase(const std::vector<StringArray> &DSNCorrectionDB)
{

   DSNCorrectionDataBase = DSNCorrectionDB;

   return true;
}

//------------------------------------------------------------------------------
// std::vector<StringArray> CelestialBody::GetDSNCorrectionDatabase()
//------------------------------------------------------------------------------
/**
* Accessor used to get the DSNCorrectionDatabase parameter of the object.
*
*
* @return true if executed correctly
*
...
std::vector<StringArray>& CelestialBody::GetDSNCorrectionDatabase()
{
   return DSNCorrectionDataBase;
}

//------------------------------------------------------------------------------
// const StringArray& CelestialBody::GetStringArrayParameter(const std::string &label) const
//------------------------------------------------------------------------------
/**
      ...
      //      case Gmat::DE430 :
   {
      if (!theSourceFile)
      {
         ...
         MessageInterface::ShowMessage
            ...
            overrideTime ? "true" : "false");
      #endif
      Real *posVel = theSourceFile->GetPosVel(bodyNumber, atTime, overrideTime);
      state.Set(posVel[0], posVel[1], posVel[2],
         posVel[3], posVel[4], posVel[5]);
      break;
   }
   ...
   else
   {
      if (isFirstTimeMu)
      {
         if (userDefined)
         {
            if (!userSetMu)
            {
               MessageInterface::ShowMessage("WARNING: Mu has not been set for the user defined body \"%s\". Using default: %.1f km^3/s^2\n", instanceName.c_str(), GmatStringUtil::ToString(mu).c_str());
            }
         }
//         MessageInterface::LogMessage
...
{   
   ...
   else
   {
      if (isFirstTimeRadius)
      {
         if (userDefined)
         {
            if (!userSetEquitorialRadius)
            {
               MessageInterface::ShowMessage("WARNING: Equatorial Radius has not been set for the user defined body \"%s\". Using default: %.1f km\n", instanceName.c_str(), GmatStringUtil::ToString(equatorialRadius).c_str());
            }
         }
 ...
 */
//------------------------------------------------------------------------------
Real CelestialBody::GetFlattening()
{
   if (isFirstTimeFlattening)
   {
      if (userDefined)
      {
         if (!userSetFlattening)
         {
            MessageInterface::ShowMessage("WARNING: Flattening has not been set for the user defined body \"%s\". Using default: %.1f \n", instanceName.c_str(), GmatStringUtil::ToString(flattening).c_str());
         }
      }
      isFirstTimeFlattening = false;
   }
   return flattening;
}
...
Real CelestialBody::SetRealParameter(const Integer id, const Real value)
{
   ...
   if (id == EPOCH_PARAM)
   {
      ...
      // Save the run epoch so a local setting overrides the global one until the global one changes
      runEpoch = GmatGlobal::Instance()->GetCurrentEpoch();
...
//      }
   ...
   if (id == DSN_DIRECTORY)
   {
      ...
      {
         DSNMediaFileDirectories.clear();
         return true;
      }

      // add name to the list
      if (find(DSNMediaFileDirectories.begin(), DSNMediaFileDirectories.end(), value) == DSNMediaFileDirectories.end())
      {
         bool hasNoPath = GmatFileUtil::HasNoPath(value);
         if (!hasNoPath)
         {
            if (this->GetFullName() == "Earth")
            {
               DSNMediaFileDirectories.push_back(value);
               return true;
            }
            else
               throw SolarSystemException("Error: DSNMediaFileDirectories must be assigned to the Earth. " + this->GetFullName() +" is not a supported body.\n");
         }
         else
            throw SolarSystemException("Error: \""+ value +"\" is not a valid directory.\n");
      }
      else
         throw SolarSystemException("Error: File name is replicated ('" + value + "')\n");
   }
...
{
   if (id == DSN_DIRECTORY)
      return DSNMediaFileDirectories;
...
//   }
   ...
   if (id == DSN_DIRECTORY)
      return DSNMediaFileDirectories.size() == 0;
                                     ...
                                     const std::string &value)
{
   ...
   if (id == VIEW_3D_MODEL_OFFSET_X || id == VIEW_3D_MODEL_OFFSET_Y || id == VIEW_3D_MODEL_OFFSET_Z ||
         ...
         if (!(kernelReader->IsLoaded(attitudeSpiceKernelNames.at(ii))))
         {
            ...
            catch (UtilityException &ue)
            {
                   ...
                   pckName.find("\\") == pckName.npos)
               {
                  ...
                  attitudeSpiceKernelNames.at(ii) = pckName;
               }
               else
               {
                  MessageInterface::ShowMessage("ERROR loading kernel %s\n",
