...
//------------------------------------------------------------------------------
void EphemerisWriter::SetOrbitData(Real epochInDays, Real state[6], Real cov[21], Real accel[3])
{
...
void EphemerisWriter::HandleWriteOrbit()
{
   ...
   if (interpolateInitialState)
   {
      ...
      #endif
      WriteOrbitAt(nextReqEpochInSecs, currState, currCov, currAccel);
   ...
   {
      if (currEpochInDays < finalEpochA1Mjd)
         WriteOrbit(currEpochInSecs, currState, currCov, currAccel);
      else
         WriteOrbitAt(nextReqEpochInSecs, currState, currCov, currAccel);
   }
   else
 ...
 */
//------------------------------------------------------------------------------
void EphemerisWriter::WriteOrbit(Real reqEpochInSecs, const Real state[6], const Real cov[21], const Real accel[3])
{
   #ifdef DEBUG_EPHEMFILE_WRITE
   MessageInterface::ShowMessage
   ...
   Real stateToWrite[6], covToWrite[21], accelToWrite[3];
   for (int i=0; i<6; i++)
   ...
   WriteOrbitData(outEpochInSecs, stateToWrite, covToWrite, accelToWrite);
 ...
 */
//------------------------------------------------------------------------------
void EphemerisWriter::WriteOrbitAt(Real reqEpochInSecs, const Real state[6], const Real cov[21], const Real accel[3])
{
}
...
//------------------------------------------------------------------------------
void EphemerisWriter::WriteOrbitData(Real reqEpochInSecs, const Real state[6], const Real cov[21], const Real accel[3])
{
   #ifdef DEBUG_EPHEMFILE_WRITE
   DebugWriteTime
   ...
   Real outState[6], outCov[21], outAccel[3];
   for (int i = 0; i < 6; i++)
      outState[i] = state[i];
   for (int i = 0; i < 21; i++)
      outCov[i] = cov[i];
   ...
   {
      ConvertState(reqEpochInSecs/GmatTimeConstants::SECS_PER_DAY, state, outState, cov, outCov, accel, outAccel);
   }
   ...
   BufferOrbitData(reqEpochInSecs / GmatTimeConstants::SECS_PER_DAY, outState, outCov, outAccel);
                                 ...
                                 Real outAccel[3])
{
   ...
   // Convert spacecraft position from Frame1 to Frame2. It needs to translation and then a rotation.
   // This is equation represtion for conversion between 2 frames:
   //             posNew = posO2 - posO1 + [Rot]*pos
   // Where: posO1 and posO2 are position of the origins of Frame1 and Frame2 respectively.
   ...
   //        newAcceleartion is the acceleration of spacecraft as it is presented in Frame2.

   Rvector3 accelO1 = dataCoordSystem->GetOrigin()->GetMJ2000Acceleration(A1Mjd(epochInDays));
   Rvector3 accelO2 = outCoordSystem->GetOrigin()->GetMJ2000Acceleration(A1Mjd(epochInDays));

   // Currently, all build in GMAT coordinate system does not calculate the second derivative of rotation matrix.
   // To simplify, we assume it is a zero matrix.
   Rmatrix33 r2dot;
