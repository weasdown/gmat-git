...
//------------------------------------------------------------------------------
void EphemWriterSPK::BufferOrbitData(Real epochInDays, const Real state[6], const Real cov[21], const Real accel[3])
{
...
void EphemWriterSPK::HandleSpkOrbitData(bool writeData, bool timeToWrite)
{
   ...
   if (writeData)
   {
      ...
      if (bufferData)
      {
         ...
         BufferOrbitData(currEpochInDays, outState, outCov, outAccel);
