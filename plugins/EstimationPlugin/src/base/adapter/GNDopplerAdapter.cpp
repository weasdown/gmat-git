...
#endif
   typeName = "RangeRate";
   parameterCount = GNDopplerAdapterParamCount;
   ...
   multiplierE            (da.multiplierE)
{
   ...
   if (da.adapterS)
   {
      ...
      adapterS = (GNRangeAdapter*)da.adapterS->Clone();
   }
}
...
void GNDopplerAdapter::SetSolarSystem(SolarSystem *ss)
{
   ...
   GNRangeAdapter::SetSolarSystem(ss);
}
...
std::string GNDopplerAdapter::GetParameterText(const Integer id) const
{
   ...
   return GNRangeAdapter::GetParameterText(id);
}
...
Integer GNDopplerAdapter::GetParameterID(const std::string& str) const
{
   ...
   return GNRangeAdapter::GetParameterID(str);
}
...
Gmat::ParameterType GNDopplerAdapter::GetParameterType(const Integer id) const
{
   ...
   return GNRangeAdapter::GetParameterType(id);
}
...
bool GNDopplerAdapter::SetStringParameter(const Integer id, const std::string& value)
{
   ...
   retval = GNRangeAdapter::SetStringParameter(id, value) && retval;
      ...
      const std::string& value, const Integer index)
{
   ...
   retval = GNRangeAdapter::SetStringParameter(id, value, index) && retval;
...
Integer GNDopplerAdapter::SetIntegerParameter(const Integer id, const Integer value)
{
   ...
   return GNRangeAdapter::SetIntegerParameter(id, value);
}
...
Real GNDopplerAdapter::GetRealParameter(const Integer id) const
{
   ...
   return GNRangeAdapter::GetRealParameter(id);
}
...
Real GNDopplerAdapter::SetRealParameter(const Integer id, const Real value)
{
   ...
   retval = GNRangeAdapter::SetRealParameter(id, value) && retval;
   return retval;
}
...
bool GNDopplerAdapter::SetBooleanParameter(const Integer id, const bool value)
{
   ...
   retval = GNRangeAdapter::SetBooleanParameter(id, value) && retval;
   return retval;
}
      ...
      const std::string& oldName, const std::string& newName)
{
   ...
   retval = GNRangeAdapter::RenameRefObject(type, oldName, newName) && retval;
      ...
      const UnsignedInt type, const std::string& name)
{
   ...
   retval = GNRangeAdapter::SetRefObject(obj, type, name) && retval;
      ...
      const UnsignedInt type, const std::string& name, const Integer index)
{
   ...
   retval = GNRangeAdapter::SetRefObject(obj, type, name, index) && retval;
...
bool GNDopplerAdapter::SetMeasurement(MeasureModel* meas)
{
   ...
   return GNRangeAdapter::SetMeasurement(meas);
}
      ...
      std::map<std::string, StringArray> *spMap)
{
   ...
   GNRangeAdapter::SetPropagators(ps, spMap);
}
      ...
      bool forSimulation)
{
   ...
   if (calcData->CheckEpochValidity(computeEpoch))
   {
      ...
      else if ((measDataE.unfeasibleReason.at(0) == 'H') || (measDataS.unfeasibleReason.at(0) == 'H'))
      {
         if (measDataS.unfeasibleReason.at(0) == 'H')
         {
            cMeasurement.unfeasibleReason = measDataS.unfeasibleReason;
            cMeasurement.isFeasible = false;
            cMeasurement.feasibilityValue = measDataS.feasibilityValue;
         }
      }
	   ...
	   for (UnsignedInt i = 0; i < paths.size(); ++i)          // In the current version of GmatEstimation plugin, it has only 1 signal path. The code has to be modified for multiple signal paths.
	   {
		   ...
		   if (USE_TAYLOR_SERIES || USE_CHEBYSHEV_DIFFERENCE)
		   {
			   ...
			   for (UnsignedInt j = 0; j < measDataS.rangeVecs.size() && j < measDataE.rangeVecs.size(); j++)
			   {
				   ...
				   {
					   Rvector3 originDelta;
					   Rvector3 chebyDelta;

					   SpacePoint *tCSOrigin = measDataS.tCSOrigins[j];
					   SpacePoint *rCSOrigin = measDataS.rCSOrigins[j];

					   Rvector3 deltaR = *measDataE.rLocs[j] - *measDataS.rLocs[j];
					   Rvector3 deltaT = *measDataE.tLocs[j] - *measDataS.tLocs[j];

					   if (tCSOrigin->IsOfType(Gmat::CELESTIAL_BODY))
					   {
						   originDelta = ((CelestialBody*)tCSOrigin)->GetPositionDeltaSSB(measDataS.tPrecTimes[j], measDataE.tPrecTimes[j]);
						   chebyDelta += -originDelta;
					   }
					   else
					   {
						   throw MeasurementException("Unable to calculate Chebyshev difference for \"" +
							   tCSOrigin->GetName() + "\", the coordinate system's origin of each signal participant "
							   "must be a CelestialBody for Chebyshev differencing.");
					   }

					   if (rCSOrigin->IsOfType(Gmat::CELESTIAL_BODY))
					   {
						   originDelta = ((CelestialBody*)rCSOrigin)->GetPositionDeltaSSB(measDataS.rPrecTimes[j], measDataE.rPrecTimes[j]);
						   chebyDelta += originDelta;
					   }
					   else
					   {
						   throw MeasurementException("Unable to calculate Chebyshev difference for \"" +
							   rCSOrigin->GetName() + "\", the coordinate system's origin of each signal participant "
							   "must be a CelestialBody for Chebyshev differencing.");
					   }
		   ...
		   cMeasurement.correction[i] = measDataE.correction[i] * multiplierE - measDataS.correction[i] * multiplierS;
			   ...
			   if (!rangeOnly)
			   {
				   ...
				   if (addNoise)
				   {
					   ...
					   if (cMeasurement.unfeasibleReason != "R")
					   {
						   ...
						   Real noiseVal = val - cMeasurement.value[i];
						   cMeasurement.value[i] = val;
						   // cMeasurement.value[i] = cMeasurement.value[i] + noiseVal;      // val already has noise. If we run this line, one more noise leavel is added to measurement value
						   cMeasurement.correction[i] = cMeasurement.correction[i] + noiseVal;
					   }
				   }
      ...
      GmatBase* obj, Integer id)
{
   ...
   if (paramName == "Bias")
   {
      ...
      if (((ErrorModel*)obj)->GetStringParameter("Type") == "RangeRate" && checker > 1)
      ...
      const std::string& correctionType)
{
   ...
   GNRangeAdapter::SetCorrection(correctionName, correctionType);
}
